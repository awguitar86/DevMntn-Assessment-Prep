...
const isForApple = aThroughD.shift();
...
function loop(obj) {
  for(var prop in obj){
    if(prop.indexOf('former') !== -1){
      delete obj[prop];
    }
  }
  return obj;
}
loop(anakinSkywalker);
...
var array = ['hello', 'cool', 'dude', 'lion', 'cool'];
...
//15. Write a function called numChecker that takes in a parameter and checks to see if it is a number or a
  ...
  var numChecker = (num) => +num || +num === 0 ? true : false;
  numChecker('hello');
